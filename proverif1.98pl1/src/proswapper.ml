...
let rec annotate = function
  ...
  | Barrier(n,tag,p,occ) ->
      ...
      let a = Terms.create_name (Terms.fresh_id "a") ([],Param.channel_type) true in
      let c = Terms.create_name (Terms.fresh_id "c") ([],Param.channel_type) true in
      Param.freenames := a::c::(!Param.freenames);
      let tag' =
	match tag with
	  None -> Terms.fresh_id "@"
	| Some s ->
...
let rec eq_proc ren1 ren2 p1 p2 =
  ...
  | Restr(f1,_,p1',_), Restr(f2,_,p2',_) ->
      ...
      (let f' = Terms.create_name (Terms.fresh_id f1.f_name) ([],snd f1.f_type) true in
...
(* The main proswapper function:
   [compile_barriers next_f p] calls [next_f] on each process obtained by
   compiling the barriers in [p], with swapping. *)

let compile_barriers next_f p =
...
(* The main compilation function for barriers for correspondence assertions:
   [compile_barriers p] returns the list of processes obtained by
   compiling the barriers in [p], with swapping. *)
