  ...
  if success then prev_state else
	...
	in
	  ...
	  match (fstatus.end_status, prev_state.goal) with
	    (NonInj, Fact(Pred(pr,[t']), _)) when pr == Param.end_pred -> do_test t'
	  | (Inj, Fact(Pred(pr,[_;t']), _)) when pr == Param.end_pred_inj -> do_test t'
	...
	try
	  auto_cleanup_red (fun () ->
	    ...
	    made_forward_step := true;
	    let success =
	      match prev_state.goal, prev_state.current_phase with
		Fact(Pred({p_info = [Table(i)]},[tbl_elem]), _), i' ->
...
let test_success cur_state' =
  try
    match cur_state'.goal, cur_state'.current_phase with
      Fact(Pred({p_info = [Attacker(i,_)]},[t]), recipe_lst), i' ->
      (* compute the new recipe_lst
        ...
        | Some recipe when i' <= i -> recipe_lst := Some [recipe]; true
        | _ -> false);
    | Fact(Pred({p_info = [Mess(i,_)]},[tc;t]), recipe_lst), i'  ->
        (* compute the new recipe_lst
      ...
      | Some recipe1, Some recipe2 when (i' <= i) -> recipe_lst := Some [recipe1; recipe2]; true
      | _ -> false)
    | WeakSecrGoal(l,_,_,_), _ ->
      ...
      (match lopt with
      | Some l' ->
        List.iter2 (fun (x,y,z) recipe -> z := Some recipe) l l';
        true
      | None -> false)
    | NonInterfGoal(NIEqTest((t1, recipe1),(t2, recipe2))), _ ->
    ...
    | NonInterfGoal(ApplyTest(_, l)), _ ->
      ...
      (match lopt with
      | Some l' ->
        List.iter2 (fun (x, y) recipe -> y := Some recipe) l l';
        true
      | None -> false;)
    ...
    (* Note: when a clause concludes bad_pred and is a ProcessRule, then its first tag
	  ...
	  InputTag occ :: _ ->
	    ...
	    if (occ == occ_goal) &&
	      ...
	      (List.for_all2 equal_terms_modulo (extract_name_params_noneed name_params') (skip (lg-l) name_params_goal)) then
	      begin
	        loc := Some (n, List.nth cur_state.subprocess n, output_loc);
	        true
	      end
	    else false
        | _ -> false
...
let do_res_in cur_state seen_list rest_subprocess n current_cache_list name_params' new_occs facts tc pat p tc' (mess_term: term) public_status next_f =
  ...
  try
    made_forward_step := true;
    let success = success_input cur_state n new_occs name_params' mess_term (LocAttacker recipe') in
...
let do_async_res_io cur_state seen_list rest_subprocess n current_cache_list name_params' new_occs facts tc pat p tc' mess_term public_channel next_f =
  ...
  begin
    try
      made_forward_step := true;
      let success = success_input cur_state n new_occs name_params' mess_term
	  ...
	  auto_cleanup_red (fun () ->
	    ...
	    let attack_found =
	      match cur_state.goal with
	        Fact(Pred({p_info = [Mess(n,_)]},[tcg;tg]), _) when (n == cur_state.current_phase) &&
		  ...
		  (* The adversary is passive and the channel is public;
                ...
                let (new_recipe, cur_state') = add_public cur_state mess_term in
	        {
	          cur_state'
	            ...
	            comment = RIO(n, tc', pat, noutput, tc', Some new_recipe, mess_term, attack_found);
	        ...
	        { cur_state
	            ...
	            subprocess = replace_at noutput (Nil, name_params2,occs2, facts2, Nothing)
	            (List.rev_append seen_list ((p', name_params'', new_occs, fact' :: facts, Nothing) :: rest_subprocess));
	            comment = RIO(n, tc', pat, noutput, tc', None, mess_term, attack_found);
	            previous_state = Some cur_state }
	    in
	  ...
	  let cur_state2 =
		  ...
		  the adversary eavesdrops the message sent by RIO2 *)
	        ...
	        comment = RIO2(n, tc', pat, noutput, tc', Some new_recipe, mess_term, attack_found);
	        previous_state = Some cur_state }
            else
	      { cur_state with
                subprocess = replace_at noutput' (Nil, name_params2,occs2, facts2, Nothing) (List.rev_append seen_list rest_subprocess);
	        comment = RIO2(n, tc', pat, noutput, tc', None, mess_term, attack_found);
	        previous_state = Some cur_state }
          in
          (* Then do RNil on the Nil process that follows the output *)
   	  let cur_state'' =
	...
	try
	  ...
	  (* When p2 is Nil and the Horn clause derivation does not justify the
	       ...
	       - when the communication itself is what makes the attack succeed,
	         ...
	         (attack_found is true in this case) *)
	  if not ((p2 != Nil) || public_channel || attack_found) then
	    raise Unify;
	  made_forward_step := true;
	  let success = success_input cur_state n new_occs name_params' t2
	      (LocProcess(noutput, List.nth cur_state.subprocess noutput))
	  in
	  if success then cur_state else
	  ...
	  try
		  ...
		  let (new_recipe, cur_state') = add_public cur_state t2 in
	            ...
	            comment = RIO(n, tc', pat, noutput, tc2, Some new_recipe, t2, attack_found);
		    previous_state = Some cur_state }
                else
		  { cur_state with
		    subprocess = replace_at noutput (p2, name_params2, (OutputTag out_occ)::occs2, facts2, Nothing)
			    (List.rev_append seen_list ((p', name_params'', new_occs, fact :: facts, Nothing) :: rest_subprocess));
		    comment = RIO(n, tc', pat, noutput, tc2, None, t2, attack_found);
		    previous_state = Some cur_state }
	      in
	      if attack_found then
		cur_state2
		...
		{ cur_state2 with
		  ...
		  comment = RIO2(n, tc', pat, noutput, tc2, Some new_recipe, t2, attack_found);
                  previous_state = Some cur_state }
	      else
		{ cur_state with
                  subprocess = replace_at noutput' (p2, name_params2, occs2, facts2, Nothing)
			  (List.rev_append seen_list rest_subprocess);
		  comment = RIO2(n, tc', pat, noutput, tc2, None, t2, attack_found);
                  previous_state = Some cur_state }
	    in
	    if attack_found then
	      (* SUCCESS! *)
...
let rec find_phase next_f cur_state n = function
  ...
  | (Phase(n,p,_),name_params,occs, facts, cache_info)::rest_subprocess ->
	...
	let cur_state'' = reclose_public cur_state' in
...
let reduction_step next_f state =
  ...
  with No_result ->
    find_phase next_f state 0 state.subprocess
...
let rec build_event_table event_table state =
  let event_table' =
  ...
  match state.previous_state with
    None -> event_table'
  | Some prev_state -> build_event_table event_table' prev_state

(* [global_event_table] contains the events on the part of
   the trace on which we are not going to backtrack
   (in case we do not backtrack on the whole trace) *)
let global_event_table = ref []

(* [add_events_previous state] adds the events found
   in the trace [state] except the last step to [global_event_table] *)
let add_events_previous state =
  match state.previous_state with
    None -> ()
  | Some s -> global_event_table := build_event_table (!global_event_table) s
...
let rec reduction_nobacktrack state =
  ...
  with Reduced one_red_state ->
    ...
    add_events_previous one_red_state;
...
let analyze_non_interf_tree tree =
  match tree.desc with
	...
	{ thefact = Pred(_, [t]) } -> (rev_name_subst t, ref None)
	  ...
	  NIEqTest((rev_name_subst t1, ref None), (rev_name_subst t2, ref None))
...
let check_disjunct restwork event_table disjunct =
  ...
  find_event_list (fun event_table' ->
    TermsEq.unify_modulo_list (fun () ->
	 ...
	 The definition is these predicates only depends on clauses
...
let all_events check_query_concl ev0 event_table =
  let param =
    match ev0 with
      QSEvent(_, param) -> param
    | _ -> Parsing_helper.internal_error "event expected in Reduction.all_events"
  in
...
let is_fresh_name f =
  match f.f_cat with
  | Name _ -> not (List.memq f (!Param.freenames))
  | _ -> false
...
let has_name_q (Before(e,hyp)) =
  (has_name_e e) || (List.exists (List.exists (function
      QEvent e' | NestedQuery(Before(e',_)) -> has_name_e e')) hyp)
...
let check_query_falsified q final_state =
  (* Include in [event_table]
     the events in [final_state], that is, those of the last chunk of the trace, and
     the events in [global_event_table], that is, the events of the previous chunks of the trace.
     ([global_event_table] is non-empty only when we do not backtrack on the full trace.) *)
  let event_table = build_event_table (!global_event_table) final_state in
  ...
  List.iter (fun (t,_) ->
    ...
    *)
  let Before(ev0, hyp) = q in
  match ev0 with
    QSEvent _ ->
      (* Check that the query is false *)
      begin
	try
	  Terms.auto_cleanup (fun () ->
  ...
  | QNeq _ | QEq _ ->
      Parsing_helper.internal_error "Inequalities or equalities cannot occur before ==> in queries"
...
let is_inj = function
    Some (Before(QSEvent(inj,_),_)) -> inj
  | _ -> false
...
let do_reduction opt_query tree =
  ...
  let goal =
	...
	WeakSecrGoal(!accu, compute_term, w1, w2)
      ...
      Fact(rev_name_subst_fact tree.thefact, ref None)
  in
  let init_state =
    { goal = goal;
  ...
  let res =
    begin
	  ...
	  begin
        ...
        let state = reclose_public state in
