...
(* Ask for a recipe for the term t. Add (recipe, t) to !cur_state.public if the recipe is found.
   Return the new state ande the expansion of the recipe if the evaluation doesn't fail.
   otherwise raise WrongChoice *)
let rec get_recipe_of_term title question_text t =
    let state = Menu_helper.get_state () in
    try
      auto_cleanup (fun () ->
        let recipe = Menu_helper.parse_term (Menu_helper.dialog_box title question_text ()) in
        let expand = Evaluation_helper.term_evaluation_fail (Menu_helper.expand_recipe state.public recipe) in
        if Reduction_helper.equal_terms_modulo t expand then
          {(Evaluation_helper.add_public_with_recipe state (recipe, expand)) with
            comment = RAddpub;
    ...
    | Menu_helper.WarningsAsError -> get_recipe_of_term title question_text t
...
let get_previous_state already_public state =
...
(* If [already_public] is true, the this function is used when the canal is public. *)
...
let rec do_public_input state n tin pat p already_public =
  let get_recipe_and_expand state =
    try
      auto_cleanup (fun () ->
        let c = Menu_helper.get_recipe "" ("Give recipe of message to input on channel " ^ (Display_interact.GtkInteract.display_term tin) ^ ":")  in
        let exp = Menu_helper.expand_recipe state.public c in
...
let do_insert state n t p =
  let s_t = Display_interact.GtkInteract.display_term t in
  let rec insert_in acc t = function
      [] -> List.rev (t::acc)
    | hd::tl as l ->
       let s_hd = Display_interact.GtkInteract.display_term hd in
       if s_hd < s_t then
         insert_in (hd::acc) t tl
       else
         if s_hd = s_t then
           List.rev_append acc l
         else
           List.rev_append acc (t::l)
  in
  try
    begin
      auto_cleanup
...
let do_restr state na env args p n =
  ...
  let need_list = Reduction_helper.get_need_vars na.f_name in
  let include_info = Reduction_helper.prepare_include_info env args need_list in
  let nt = FunApp(na, Reduction_helper.extract_name_params na.f_name include_info []) in
  let n' = FunApp(Reduction_helper.add_name_for_pat nt,[]) in
...
let do_rio t pat p q n_input n_output tin state =
  debug_print ("n_input " ^ string_of_int n_input ^ " n_outpout " ^ string_of_int n_output);
...
let show_terms termsl =
  ...
  scrolled_window#misc#modify_bg [(`NORMAL, `WHITE)];
  ...
  let cancel_b = GButton.button ~label:"Cancel"  ~packing:window#action_area#add () in
...
let do_manual_reduction state n proc =
  ...
  let quest_io io tin set_tio func =
    ...
    begin
      match ans with
      | 1 ->
         let n_state = get_recipe_of_term ("Private " ^ io)  ("Give a recipe for the channel " ^ Display_interact.GtkInteract.display_term tin) tin in
         func n_state
      | 2 ->
	 ...
	 if List.exists (fun (_,x) -> Menu_helper.equal_io_oi x set_tio) data.titles then
           ...
           let _ = Menu_helper.question_box "Error" ["OK"] "No Reduction possible on this private channel" () in
           state
      | _ ->  state
    end
  in
    ...
    | Other ->
       begin
         ...
         | Get(pat, t, p, q, occ) ->
            ...
            let match_terms_lst = List.filter (fun term ->
              try
                Terms.auto_cleanup(fun () -> Evaluation_helper.match_pattern pat term;
                  let t' = Evaluation_helper.term_evaluation_fail t in
...
let rec do_one_reduction_step state n =
  ...
  let aux state n proc =
    if Menu_helper.is_auto_reductible state proc then
      ...
      do_manual_reduction state n proc
  in
  match proc with
  | Repl(_, _) | Par(_, _) ->
     if Menu_helper.no_more_proc () then
