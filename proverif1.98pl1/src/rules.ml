    ...
    Pred(chann, ((FunApp(f,_) :: _) as l0)) when 
      ...
      (chann.p_prop land Param.pred_TUPLE != 0) ->
        let rec tuple_make hist' f n = function
	      ...
	      let hist_dec = History.get_rule_hist (RApplyProj(f, n, chann)) in
              let hist'' = Resolution(hist', 0, hist_dec) in
              match first with 
                FunApp(f,_) :: _ when f.f_cat = Tuple -> 
		  begin
...
let rec transform_hyp preds v hist n = function
  ...
  | (f::l) ->
	...
	Pred(p, [t1; Var v']) when p.p_prop land Param.pred_ELEM != 0 
	      && v == v' ->
		let (Rule(_, _, hyp, _, _)) as hist_dec = History.get_rule_hist (RElem(preds, p)) in
...
let implies2 f (hyp1, concl1, _, constr1) (hyp2, concl2, _, constr2) =
  Terms.auto_cleanup (fun () ->
	...
	raise NoMatch;
      implies_constra_list (concl2 :: hyp2) constr2 constr1 ();
      f match_done)
...
let completion rulelist =
  ...
  begin
    ...
    if !Param.html_output then
      begin
	Display.Html.print_string "<H2>Clauses completed with equations</H2>\n";
	Display.Html.display_item_list Display.Text.display_rule_nonewline rulelisteq
      end
