...
let rename_internal_private_name n =
  Terms.create_name n.f_name n.f_type true
...
let rec copy_process add_in_glob_table = function
  ...
  | Restr(n,(args,env),p,occ) ->
	...
	Restr(n', (new_args,update_env env), Reduction_helper.process_subst (copy_process add_in_glob_table p) n (FunApp(n',[])), new_occurrence())
      else
  ...
  | Event(t, (args, env), p, occ) ->
      let (new_args,lets) = update_args_opt args in
      Terms.put_lets
	(Event(Terms.copy_term3 t, (new_args,update_env env), copy_process add_in_glob_table p, new_occurrence())) lets
  ...
  | Barrier(n,tag,p,occ) -> Barrier(n, tag, copy_process add_in_glob_table p, new_occurrence())
  | AnnBarrier _ ->
...
(* Create a fresh copy of the process with occurrences nicely numbered.
   Names and variables are fresh copies but keep their old name *)

let reset_occurrence p =
  Terms.auto_cleanup (fun () ->
     Terms.reset_occurrence();
     copy_process false p
   )
...
let obtain_biprocess_from_processes process_1 process_2 next_f =
  ...
  let process = Test(cond,process_1,process_2,new_occurrence ()) in

  simplify_process process next_f
